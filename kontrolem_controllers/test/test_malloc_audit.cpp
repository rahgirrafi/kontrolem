...
int main()
{
  ...
  // LPV: a grid of LQR designs over the pole angle + multilinear interpolation. The
  // per-tick path is a 4-corner (here 2-corner, 1 axis) blend into preallocated
  // buffers — must be malloc-free like the plain gemv.
  LpvController lpv({"cart_joint"}, Q, Eigen::MatrixXd::Identity(1, 1),
                    {SchedAxis{1, -1.0, 1.0, 11}});
  lpv.configure(model, *lpv.synthesize(model, upright), upright);
  ...
  const long lpv_m = count_mallocs(lpv, s, upright, N);
  ...
  std::cout << "  LpvController          : " << lpv_m << "  (" << (double)lpv_m / N
            << "/call)  <- gridded LQR + multilinear interp\n";
