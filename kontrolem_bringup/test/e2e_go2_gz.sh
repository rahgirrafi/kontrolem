#!/usr/bin/env bash
# End-to-end WBC-under-real-contact validation for the real Unitree Go2 (M7). The
# SAME KontrolemController (control_law: wbc) that stands the toy floating_quadruped
# now stands the Go2 against Gazebo Fortress's OWN contact/friction solver: joints via
# ign_ros2_control/IgnitionSystem, base pose/twist + contact via
# kontrolem_gz/GzBaseStateSystem (read from the ECM). Only the robot (model + config)
# differs from e2e_quad_gz.sh — zero controller-code change.
#
# Sequence: spawn WELDED to a static anchor (a DetachableJoint freezes the whole model
# in DART — the "hold until commanded" analogue), WBC activates against the frozen
# robot, DETACH hands it a clean nominal state, assert it holds the stance on real
# contact, then shove the base (one-shot ApplyLinkWrench) and assert it recovers.
# base_height 0.2868 is offline-proven (feet grounded). Headless; standalone; timeout.
#
# Usage: e2e_go2_gz.sh [push_N] [run_s]
set -o pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
PUSH="${1:-5000.0}"       # one-shot lateral force on the base (N, for one 2 ms step);
                          # Go2 (~15 kg) needs more than the toy's 3000 N for a ~6 cm shove
RUN="${2:-60}"
...
sleep 15   # gazebo + spawn + weld + WBC activation (robot frozen at nominal)
...
# base_height 0.2868; allow settle band [0.25, 0.33].
stance = 0.25 <= s["z_min"] and s["z_final"] <= 0.33 and abs(s["y_final"]) < 0.03 and s["ok_rate"] > 0.99
pushed = 0.02 <= abs(p["y_peak"]) <= 0.15     # visibly shoved, but in-basin
recovered = abs(p["y_final"]) < 0.03 and 0.25 <= p["z_final"] <= 0.33 and p["ok_rate"] > 0.99
ok = stance and pushed and recovered
print("%s: Gazebo Go2 WBC — stance(z=%.3f, ok=%.3f)=%s push(y_peak=%.3f)=%s recovery(y=%.3f z=%.3f ok=%.3f)=%s" % (
