...
//
// ROS-free, Eigen-only. Concrete sources (e.g. CrawlGait) are built from the nominal foot
// positions (FK once, off the RT path) + gait parameters, so sample() needs no model.
...
/// caller's pre-sized buffer and must not allocate.
struct GaitSource
{
