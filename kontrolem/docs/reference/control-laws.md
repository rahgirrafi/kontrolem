...
| | |
|---|---|
| Method | Floating-base inverse-dynamics QP over `[q̈; λ; τ]`: dynamics equality `M q̈ + h = Sᵀτ + Jᵀλ`, no-slip contact `J q̈ = −γ`, friction pyramid `|λ_xy| ≤ μλ_z` + `λ_z ≥ 0`, torque limits; cost = frame-consistent PD task `q̈_des = −Kp(q ⊖ q_ref) − Kd v` + force/torque regularization. OSQP seam (tolerance 1e-4). |
| Accepts | Regulation |
| Required state | Floating-base state: SE(3) base pose/twist + joint pos/vel (+ contact schedule). |
| Heavy phase | none (`synthesize()` is a no-op). |
| `compute()` | Query dynamics + stacked contact Jacobian + contact drift, build + solve the QP, output `τ`. |
| `status().ok` | QP solved. |
...
| Scope | Standing / push-recovery with a scheduled all-stance contact set; no locomotion. |
...
| `wbc` | Regulation | floating base + contacts | none | yes (QP) |
