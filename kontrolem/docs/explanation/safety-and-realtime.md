...
So on the WBC path, allocation-free `compute()` is now *certified* by the audit. The remaining honest gap for **hard** real-time is the solver's iteration count, which is still data-dependent — a hard-RT deployment must additionally cap OSQP's `max_iter`. Firm real-time is demonstrated; hard real-time is architecturally reachable and now allocation-clean, with the iteration cap the remaining piece. See [Design decisions](design-decisions.md) for the full register.
