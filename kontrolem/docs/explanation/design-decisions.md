...
- **The whole-body `compute()` is not yet allocation-certified.** One known allocation remains (forming the SE(3) posture error). Firm real-time is demonstrated; hard real-time on the QP path is designed-for, not certified — see [Safety & real-time](safety-and-realtime.md).
