...
Three consumers want a contact signal: the WBC (which feet can push), a future state estimator, and future gait switching. In this version the **simulator provides it as ground truth** (all feet in stance), and the WBC uses a **scheduled all-stance** set. That is enough for standing and push recovery, which need no contact *detection*. Real contact *estimation* (e.g. from joint torques) is deliberately deferred — see [Design decisions](design-decisions.md#whats-deliberately-not-done). Treating contact as an explicit, integrator-supplied contract boundary (like the base estimator) is the intended path to hardware.
