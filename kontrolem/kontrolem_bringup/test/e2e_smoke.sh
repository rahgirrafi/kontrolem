...
#
# Usage: e2e_smoke.sh <launch_file> <joint> <threshold> [settle_s] [run_s]
...
# abs(VAL) < THRESH ?
python3 - "$VAL" "$THRESH" "$JOINT" <<'PY'
import sys
val, thr, joint = float(sys.argv[1]), float(sys.argv[2]), sys.argv[3]
ok = abs(val) < thr
print(f"{'PASS' if ok else 'FAIL'}: {joint} settled to {val:+.4f} (|.| < {thr})")
